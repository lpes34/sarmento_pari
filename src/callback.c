...
void on_mouse_button_on(GtkWidget *widget, GdkEventButton *event, gpointer user_data)
{
  if( event->button == 2 )
        ...
        /*...*/  //do not forget to force refresh of the widget - draw
...
gboolean p_ForceRefreshDA(gpointer user_data)
{
        GtkWidget *da;
        if( ! user_data)
           da = GTK_WIDGET(gtk_builder_get_object (builderG, "GtkDrawingArea1"));
        else
...
void p_InitTimer()
{
  ...
  GtkWidget *da=GTK_WIDGET(gtk_builder_get_object (builderG, "GtkDrawingArea1"));
